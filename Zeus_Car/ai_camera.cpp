...
void AiCamera::begin(const char *ssid, const char *password, const char *wifiMode, const char *wsPort)
{
  ...
  char _eeprom_name[NAME_EEPROM_MAX_LEN];
  char _eeprom_wifi_mode[WIFI_MODE_EEPROM_MAX_LEN];
  char _eeprom_ssid[SSID_EEPROM_MAX_LEN];
  char _eeprom_psk[PSK_EEPROM_MAX_LEN];
  char _eeprom_have_saved;

  // check there is wifi information saved in EEPROM
  EEPROM.get(HAVE_DATE_EEPROM_ADDRESS, _eeprom_have_saved);
  if (_eeprom_have_saved == 'Y') // if has saved, read param from EEPROM
  {
    // read param from EEPROM
    EEPROM.get(NAME_EEPROM_ADDRESS, _eeprom_name);
    EEPROM.get(WIFI_MODE_EEPROM_ADDRESS, _eeprom_wifi_mode);
    // eeprom_read_string(WIFI_MODE_EEPROM_ADDRESS, WIFI_MODE_EEPROM_MAX_LEN, _eeprom_wifi_mode);
    EEPROM.get(SSID_EEPROM_ADDRESS, _eeprom_ssid);
    // eeprom_read_string(SSID_EEPROM_ADDRESS, SSID_EEPROM_MAX_LEN, _eeprom_ssid);
    EEPROM.get(PSK_EEPROM_ADDRESS, _eeprom_psk);
    // eeprom_read_string(PSK_EEPROM_ADDRESS, PSK_EEPROM_MAX_LEN, _eeprom_psk);

    DebugSerial.print(F("eeprom size: "));
    DebugSerial.println(EEPROM.length());

    DebugSerial.print(F("eeprom_name: "));
    DebugSerial.println(_eeprom_name);

    DebugSerial.print(F("_eeprom_wifi_mode: "));
    DebugSerial.println(_eeprom_wifi_mode);

    DebugSerial.print(F("_eeprom_ssid: "));
    DebugSerial.println(_eeprom_ssid);

    DebugSerial.print(F("_eeprom_psk: "));
    DebugSerial.println(_eeprom_psk);

    DebugSerial.print(F("_eeprom_have_saved: "));
    DebugSerial.println(_eeprom_have_saved);

    // EEPROM.put(HAVE_DATE_EEPROM_ADDRESS, 'N');
  }
  else // if not saved, write default value to EEPROM
  {
    strcpy(_eeprom_name, name);
    strcpy(_eeprom_wifi_mode, wifiMode);
    strcpy(_eeprom_ssid, ssid);
    strcpy(_eeprom_psk, password);

    EEPROM.put(NAME_EEPROM_ADDRESS, _eeprom_name);
    EEPROM.put(WIFI_MODE_EEPROM_ADDRESS, _eeprom_wifi_mode);
    EEPROM.put(SSID_EEPROM_ADDRESS, _eeprom_ssid);
    EEPROM.put(PSK_EEPROM_ADDRESS, _eeprom_psk);
    EEPROM.put(HAVE_DATE_EEPROM_ADDRESS, 'Y');
  }
  ...
  this->set("NAME", _eeprom_name);
  this->set("TYPE", type);
  this->set("SSID", _eeprom_ssid);
  this->set("PSK", _eeprom_psk);
  this->set("MODE", _eeprom_wifi_mode);
